...
__unused static void __activate_enclave_pmp(enclave_region_t *enc_reg, int pmp_count)
{
    int index = pmp_count - 1;
	// PMP_SET_IDX(index, -1UL, PMP_DISABLE, NAPOT);
	// index--;
...
void activate_lpmp(u64 eid)
{
    ...
    if (likely(pmp_enable)) {
        enclave_region_t *enc_reg = get_enclave_regions(eid);
        if (unlikely(eid == HOST_EID)) {
            __activate_host_pmp(enc_reg, max_pmp);
        } else {
            // __activate_enclave_pmp(enc_reg, max_pmp);
            if (likely(id_split)) {
                __activate_enclave_pmp(enc_reg, max_pmp);
            } else {
                __activate_host_pmp(enc_reg, max_pmp);  // disable i & d split 
            }
        }
    } else {
        set_pmp(max_pmp-1, 0, -1UL);
    }
